...
def jt_isim(c_total, n_objects):
    ...
    """
    sum_kq = jnp.sum(c_total)
    sum_kqsq = jnp.dot(c_total, c_total)
    a = (sum_kqsq - sum_kq)/2
...
def calc_centroid(linear_sum, n_samples):
    ...
    """
    return jnp.where(linear_sum >= n_samples * 0.5 , 1, 0)

def _iterate_sparse_X(X):
    """This little hack returns a densified row when iterating over a sparse
    matrix, instead of constructing a sparse matrix for every row that is
    expensive. #TODO: can vectorize this?
    """
...
def _split_node(node, threshold, branching_factor):
    ...
    node1_closer = node1_closer.at[farthest_idx[0]].set(True)# node1_closer[farthest_idx[0]] = True
...
class _BFNode:
    ...
    def __init__(self, *, threshold, branching_factor, is_leaf, n_features, dtype):
        ...
        self.subclusters_ = []
        self.init_centroids_ = jnp.zeros((branching_factor + 1, n_features), dtype=dtype)
    ...
    def append_subcluster(self, subcluster):
        ...
        self.init_centroids_ = self.init_centroids_.at[n_samples].set(subcluster.centroid_) # self.init_centroids_[n_samples] = subcluster.centroid_ #  #x = x.at[idx].set(y)
    ...
    def update_split_subclusters(self, subcluster, new_subcluster1, new_subcluster2):
        ...
        self.init_centroids_ = self.init_centroids_.at[ind].set(new_subcluster1.centroid_) # self.init_centroids_[ind] = new_subcluster1.centroid_
        self.centroids_ = self.centroids_.at[ind].set(new_subcluster1.centroid_) # self.centroids_[ind] = new_subcluster1.centroid_
    ...
    def insert_bf_subcluster(self, subcluster, set_bits):
        ...
        a = jnp.dot(self.centroids_, subcluster.centroid_)
        sim_matrix = a / (jnp.sum(self.centroids_, axis = 1) + set_bits - a)
        closest_index = jnp.argmax(sim_matrix)
        ...
        if closest_subcluster.child_ is not None:
            ...
            if not split_child:
                ...
                self.init_centroids_ = self.init_centroids_.at[closest_index].set(self.subclusters_[closest_index].centroid_) # self.init_centroids_[closest_index] = self.subclusters_[closest_index].centroid_
                self.centroids_ = self.centroids_.at[closest_index].set(self.subclusters_[closest_index].centroid_)# self.centroids_[closest_index] = self.subclusters_[closest_index].centroid_
                return False
        ...
        # good to go!
            ...
            if merged:
                self.centroids_ = self.centroids_.at[closest_index].set(closest_subcluster.centroid_) # self.centroids_[closest_index] = closest_subcluster.centroid_
                self.init_centroids_ = self.init_centroids_.at[closest_index].set(closest_subcluster.centroid_) # self.init_centroids_[closest_index] = closest_subcluster.centroid_
                return False
...
class JaxBitBirch():
    ...
    def _fit(self, X):
        ...
        centroids = jnp.concatenate([leaf.centroids_ for leaf in self._get_leaves()])
